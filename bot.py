...
async def category_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "income":
        add_transaction(name, uid, "income", amount, comment, "💰 Доход")
        sign = "+"
    else:
...
def main():
    ...
    conv_month = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("📆 Отчёт по месяцам"), month_start)],
        states={
