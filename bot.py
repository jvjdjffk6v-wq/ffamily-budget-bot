...
async def history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        user, ttype, amount, comment, cat = r
...
async def report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, ttype, amount, cat in rows:
        ...
        per_user.setdefault(name, {"income": 0, "expense": 0})
    ...
    for name, data in per_user.items():
        text += f"{name}: +{data['income']:,.0f} / -{data['expense']:,.0f} ₽\n"
...
# 🔥 ВАЖНО: пополнение БЕЗ категорий
async def comment_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "income":
        add_transaction(user_name, user_id, "income", amount, comment, "💰 Пополнение")
...
async def category_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    add_transaction(user_name, user_id, "expense", amount, comment, category)
